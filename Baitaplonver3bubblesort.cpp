...
// Ðinh nghia cau truc cho sinh vien 
struct SinhVien {
    string id;       // Ma so sinh vien 
    string ten;      // Ten sinh viên
...
float tinhDiemTrungBinh(SinhVien* head);
string chuyenDiemHeChu(float diemH10);

void inDanhSach(SinhVien* head);

int main() {
    SinhVien* head = NULL; // Kh?i t?o danh sách liên k?t kép
    ...
    do {
        ...
        switch (luaChon) {
            ...
            case '7':
                ...
                break;
            case '8':
                cout << "Diem trung binh cua tat ca cac sinh vien: " << tinhDiemTrungBinh(head) << endl;
                break;
            case '9':
                inDanhSach(head);
                break;
...
float tinhDiemTrungBinh(SinhVien* head) {
    if (head == NULL) {
        ...
        return 0.0; // Trong truong hop danh sach sinh vien rong, tra ve 0.0
    }
